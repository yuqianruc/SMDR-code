...
results_par <- foreach (t=1:tmax, .errorhandling = "pass", .combine=rbind, .packages=c("glmnet","RCAL","ranger")) %dopar% {
  ...
  message(paste("Iteration ",t," SMDR2 done; Time:",paste(sprintf("%.3f", time_all[2]),collapse = " ")))
  ...
  message(paste("Iteration ",t," DTL2 done; Time:",paste(sprintf("%.3f", time_all[4]),collapse = " ")))
  ...
  message(paste("Iteration ",t," done; Error:",paste(sprintf("%.3f", pred_theta_t[c(2,4,5,8,11)]-theta),collapse = " "),"; Time:",paste(sprintf("%.3f", time_all),collapse = " ")))
  result
}
str(results_par)
...
filename1=paste("predtheta_synthetic_",setting,"level",level_signal,"_part1.RData",sep="")
save(pred_theta,file=filename1)
filename2=paste("predvar_synthetic_",setting,"level",level_signal,"_part1.RData",sep="")
save(pred_sig2,file=filename2)

pred_theta=pred_theta[,c(2,4,5,8,11)]
pred_sig2=pred_sig2[,c(2,4,5,8,11)]
...
est=c("SMDR2","DTL2","IPW","SDR+","empdiff")
