...
setting="b";N=1000;d1=100;d2=50;d=d1+d2;tmax=200;K=5;shape1=1;shape2=2;M=floor(N/K);num_est=9;num_totest=num_est+1;nrho=100
logistic=function(x){exp(x)/(1+exp(x))}
#Parameters
beta_pi1_0=0
beta_pi1_S1=2*c(1,1,0,rep(0,d1-3))
beta_pi2_0_0=0
beta_pi2_0_S1=-2*c(1,1,0,rep(0,d1-3))
beta_pi2_0_S2=0*c(1,1,1,rep(0,d2-3))
beta_pi2_1_0=-beta_pi2_0_0
beta_pi2_1_S1=-beta_pi2_0_S1
beta_pi2_1_S2=-beta_pi2_0_S2
beta_g0_0=1
beta_g0_S1=1*c(1,0,0,rep(0,d1-3))
beta_g0_S2=1*c(1,1,1,rep(0,d2-3))
beta_g1_0=beta_g0_0
...
gamma_1_0=beta_g1_0+1*sum(beta_g1_S2)
gamma_0_0=beta_g0_0
theta0=beta_g0_0
theta1=beta_g1_0+sum(beta_g1_S2)
theta=theta1-theta0

#Find the normalizing parameters
sd_cal <- function(shape1, shape2) {
  mu <- shape1 / (shape1 + shape2)
  sigma2 <- shape1 * shape2 / ((shape1 + shape2)^2 * (shape1 + shape2 + 1))
  c <- mu + 0.3
  p <- 1 - pbeta(c, shape1, shape2)
  integrand <- function(u) (u - mu)^2 * dbeta(u, shape1, shape2)
  A <- integrate(integrand, lower = c, upper = 1)$value
  sqrt(2 * sigma2 * A + sigma2^2 * p)
}
sd_signal=sd_cal(shape1, shape2)
sd_beta=sqrt(shape1 * shape2 /((shape1 + shape2)^2 * (shape1 + shape2 + 1)))
...
results_par <- foreach (t=1:tmax, .errorhandling = "pass", .combine=rbind, .packages=c("glmnet")) %dopar% {
  set.seed(seeds[t]) # Set seed inside the foreach loop to ensure reproducibility
  
  # Data generation
  S1=matrix(rbeta(N*d1,shape1,shape2)-shape1/(shape1+shape2),nrow=N)
  delta2=matrix(rbeta(N*d2,1,4)-1/5,nrow=N)
  pi1=logistic(beta_pi1_0+S1%*%beta_pi1_S1)
  A1=rbinom(N,1,pi1)
  S2=S1%*%W1+1+delta2
  S2[A1==0]=S1[A1==0,]%*%W0+delta2[A1==0]
  S2bar=cbind(S1,S2)
  S2_1=S1%*%W1+1+delta2
  S2_0=S1%*%W0+delta2
  signal1=S1[,4]/sd_beta+2*(S1[,1]*S1[,2]*(S1[,2]>0.3)+S1[,1]*S1[,3]*(S1[,1]>0.3)+S1[,2]*S1[,3]*(S1[,1]>0.3))/sd_signal
  signal0=-signal1
  mu2_1=beta_g1_0+signal1+S2_1%*%beta_g1_S2
  mu2_0=beta_g0_0+signal0+S2_0%*%beta_g0_S2
  mu1_1=gamma_1_0+signal1+S1%*%W1%*%beta_g1_S2
  mu1_0=gamma_0_0+signal0+S1%*%W0%*%beta_g0_S2
  ...
  A2=rbinom(N,1,pi2)
  g=(A1+A2==0)*(beta_g0_0+signal0+S2%*%beta_g0_S2)+(A1*A2==1)*(beta_g1_0+signal1+S2%*%beta_g1_S2)
  zeta=rnorm(N)
  Y=g+zeta
...
filename1=paste("predtheta_DTE_tunning_b_N",N,"d1",d1,"d2",d2,".RData",sep="")
save(pred_theta,file=filename1)
filename2=paste("predvar_DTE_tunning_b_N",N,"d1",d1,"d2",d2,".RData",sep="")
